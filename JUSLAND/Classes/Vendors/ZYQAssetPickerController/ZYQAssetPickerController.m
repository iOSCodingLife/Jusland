...
#define kThumbnailLength    (ScreenWidth == 375) ? 90.0f : 78.0f//每张图片的宽度
#define kThumbnailGrouLength  78.0f  //组界面的高度
...
//         baselineAdjustment:UIBaselineAdjustmentAlignCenters];
    ...
    CGRect placeHolderRect = CGRectMake(0.0f,
                                        0.0f,
                                        rect.size.width - titleSize.width - 2,
                                        (height - 12) / 2);
    ...
    [self.text drawInRect:placeHolderRect
...
-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if (_delegate!=nil&&[_delegate respondsToSelector:@selector(shouldTap)]) {
        if (![_delegate shouldTap]&&!_selected) {
            if ([_delegate respondsToSelector:@selector(reachMaxSelectCount)]) {
                [_delegate reachMaxSelectCount];
            }
            return;
        }
    }
...
- (void)reachMaxSelectCount {
    if (_delegate && [_delegate respondsToSelector:@selector(reachMaxCountSelectHandleWithAsset:)]) {
        [_delegate reachMaxCountSelectHandleWithAsset:_asset];
    }
}
...
-(void)reachMaxCountSelectHandleWithAsset:(ALAsset *)asset {
    if (_delegate && [_delegate respondsToSelector:@selector(reachMaxSelectCountWithAsset:)]) {
        [_delegate reachMaxSelectCountWithAsset:asset];
    }
}
...
//
        columns = ScreenWidth > 375 ? 5:4;
        self.tableView.contentInset = UIEdgeInsetsMake(9.0, 0, 0, 0);
        minimumInteritemSpacing = (ScreenWidth-columns*kThumbnailSize.width)/(columns +1);
...
- (void)setupButtons
{
    ...
    UIBarButtonItem *pieItem = [[UIBarButtonItem alloc] initWithCustomView:button];
    self.navigationItem.rightBarButtonItem = pieItem;
    
    [self configCustomBackButton];

}
- (void)configCustomBackButton
{
    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
    button.frame = CGRectMake(0, 0, 35, 35);//40 44  20 22
...
//    [cell bind:tempAssets selectionFilter:picker.selectionFilter minimumInteritemSpacing:minimumInteritemSpacing minimumLineSpacing:minimumLineSpacing columns:columns assetViewX:(self.tableView.frame.size.width-kThumbnailSize.width*tempAssets.count-minimumInteritemSpacing*(tempAssets.count-1))/2];
    [cell bind:tempAssets selectionFilter:picker.selectionFilter minimumInteritemSpacing:minimumInteritemSpacing minimumLineSpacing:minimumLineSpacing columns:columns assetViewX:minimumInteritemSpacing];
...
//            if (vc.indexPathsForSelectedItems.count) {
//                
//            }
...
- (void)reachMaxSelectCountWithAsset:(ALAsset *)asset {
    ZYQAssetPickerController *vc = (ZYQAssetPickerController *)self.navigationController;
    vc.indexPathsForSelectedItems = _indexPathsForSelectedItems;

    if (vc.delegate && [vc.delegate respondsToSelector:@selector(reachMaxSelectCountWithPick:)]) {
        [vc.delegate reachMaxSelectCountWithPick:vc];
    }
}
...
- (void)setTitleWithSelectedIndexPaths:(NSArray *)indexPaths
{
    ...
    ZYQAssetPickerController *vc = (ZYQAssetPickerController *)self.navigationController;
//    BOOL selectable = [vc.selectionFilter evaluateWithObject:asset];
    if (indexPaths.count >= vc.maximumNumberOfSelection) {
        if (vc.delegate!=nil&&[vc.delegate respondsToSelector:@selector(assetPickerControllerDidMaximum:)]) {
            //            if (vc.indexPathsForSelectedItems.count) {
            //
            //            }
...
- (void)bind:(ALAssetsGroup *)assetsGroup
{
    ...
    float scale                 = height / kThumbnailGrouLength;
